...
int main(int argc, char* argv[]) {
	...
	try {
		...
		if (opt.verbose) {
			std::cout << "[info] cconv v0.0.0 invoked with " << argc << " arguments."
			          << std::endl;
		...
		std::unique_ptr<cnc_conversion_registry, cnc_registry_deleter> registry = nullptr;
		{
			...
			if (err != CNC_OPEN_ERROR_OKAY) {
				if (!opt.silent) {
					std::cerr
					     << "[error] Could not open a conversion registry (error code: " << err
					     << ")." << std::endl;
				}
		...
		cnc_conversion_info info                                           = {};
		{
			...
			cnc_open_error err                  = cnc_new_n(
			                      __registry, from_size, from_data, to_size, to_data, &raw_conversion, &info);
			if (err != CNC_OPEN_ERROR_OKAY) {
				if (!opt.silent) {
					std::cerr << "[error] Could not open a conversion from \"" << opt.from_code
			...
			if (opt.verbose) {
				std::cout << "[info] Opened a conversion from \""
				          ...
				          << "\" to \"" << utf8string_view(info.to_code_data, info.to_code_size)
				          << "\"";
				if (info.is_indirect) {
					std::cout
					     << " (converting indirectly from \""
					     << utf8string_view(info.from_code_data, info.from_code_size)
					     << "\" to \""
		...
		if (opt.maybe_output_file_name) {
			...
			std::string file_name
			     = std::string(reinterpret_cast<const char*>(output_file_name.data()),
			          output_file_name.size());
			if (file_name.find('\0') != std::string::npos) {
				if (!opt.silent) {
					std::cerr << "[error] File name contains embedded null: cannot open "
					             "output file \""
			...
			if (!target_output.is_open() || !target_output.good()) {
				if (!opt.silent) {
					std::cerr << "[error] Could not open output file (binary, append) \""
		...
		if (opt.verbose) {
			...
			std::cout << "[info] Attempting to convert from \"" << opt.from_code << "\" to \""
			          << opt.to_code << "\" for " << opt.input_files.size() << " inputs."
			          << std::endl;
		}

		std::size_t buffer_size
		     = opt.maybe_buffer_size.value_or(ZTD_INTERMEDIATE_BUFFER_SUGGESTED_BYTE_SIZE_I_);
		...
		const size_t initial_error_output_size         = ztd_c_array_size(error_output);
		{
			...
			for (std::size_t i = 0; i < opt.input_files.size(); ++i) {
				...
				for (; input_size > 0;) {
					...
					const cnc_mcerror convert_err    = cnc_conv(
					        raw_conversion, &output_size, &output_data, &input_size, &input_data);
					const std::size_t output_written = initial_output_size - output_size;
					switch (convert_err) {
					case CNC_MCERROR_INCOMPLETE_INPUT:
						if (!opt.silent) {
							std::cerr << "[error] Could not convert input data from \""
							          << opt.from_code << "\" to \"" << opt.to_code
							          << "\" because the input data is incomplete."
							          << std::endl;
						}
					...
					case CNC_MCERROR_INVALID_SEQUENCE: {
						...
						auto handler_visitor             = [&](auto&& arg) {
                                   return arg(info, raw_conversion, &error_output_size,
							                 &error_output_data, &input_size, &input_data);
						};
						// okay, handle it with the handler
						bool can_continue_processing
						     = std::visit(handler_visitor, opt.error_handler);
					...
					if (must_write_output) {
						output_stream.write(
						     reinterpret_cast<const char*>(initial_output_data),
						...
						if (!output_stream) {
							if (!opt.silent) {
								std::cerr
								     << "[error] Could not flush data to output file (\""
								     << output_file_name << "\")." << std::endl;
							}
							return exit_file_write_failure;
						}
						if (opt.verbose) {
							std::cout << "[info] Wrote " << output_written
							          << " bytes of output to \"" << output_file_name
							          << "\" with " << input_size << " bytes left to read."
							          << std::endl;
						}
					}
					if (must_write_error_output) {
						output_stream.write(
						     reinterpret_cast<const char*>(initial_error_output_data),
						     error_output_written);
						if (!output_stream) {
							if (!opt.silent) {
								std::cerr << "[error] Could not write data produced by the "
								             "error handler to "
								             "output file (\""
								          << output_file_name << "\")." << std::endl;
							}
						...
						if (opt.verbose) {
							std::cout << "[info] Wrote " << error_output_written
							          << " bytes of error to \"" << output_file_name
							          << "\" with " << input_size << " bytes left to read."
							          << std::endl;
						}
					}
		...
		if (!opt.silent)
			std::cerr << "[error] An explosive, unexpected error (exception) occurred and the "
	...
	if (opt.verbose) {
		std::cout << "[info] cconv completed successfully. Read a total of " << total_input_read
		          << " bytes and wrote a total of " << total_output_written << " bytes"
		          << std::endl;
	}
	return exit_success;
}
