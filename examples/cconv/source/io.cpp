...
input_read read_input_into(
     std::vector<unsigned char>& input, const input_type& any_input, bool verbose, bool silent) {
	...
	auto input_visitor = [&](auto&& arg) -> std::optional<int> {
		...
		if constexpr (std::is_same_v<utf8string, arg_type>) {
			...
			if (file_name.find('\0') != std::string::npos) {
				if (!silent) {
					std::cerr << "[error] File name contains embedded null: cannot open input "
					             "file \""
					          << input_file_name << "\" properly." << std::endl;
				}
			...
			if (!input_stream.is_open() || !input_stream.good()) {
				if (!silent) {
					std::cerr << "[error] Failure to open input file \"" << input_file_name
					          << "\"." << std::endl;
				}
				return { exit_file_open_failure };
			}
			...
			input_file_name.assign(reinterpret_cast<const ztd_char8_t*>(+"<stdin>"), 7);
			if (verbose) {
				std::cout << "[info] Attempting to read bytes from \"" << input_file_name
				          << "\"." << std::endl;
			}
		}
		else {
		...
		while (true) {
			...
			input_stream.read(
			     reinterpret_cast<char*>(input.data() + input_read), requested_read_size);
			const std::streamsize last_input_read   = input_stream.gcount();
			const bool is_successful_read           = static_cast<bool>(input_stream);
			const bool is_specific_istream_read_eof = (input_stream.eof() && input_stream.fail()
			     && (static_cast<std::size_t>(last_input_read) <= requested_read_size));
			if (is_successful_read || is_specific_istream_read_eof) {
				...
				if (!silent) {
					std::cerr << "[error] Failure to read data from \"" << input_file_name
					          << "\"." << std::endl;
				}
...
std::optional<unsigned long long> parse_unsigned_integer(utf8string_view value, int base) {
	std::string c_string_value(reinterpret_cast<const char*>(value.data()), value.size());
	try {
		unsigned long long parsed_value = std::stoull(c_string_value, nullptr, base);
