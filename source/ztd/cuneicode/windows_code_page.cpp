...
cnc_mcerr cnc_c32nrtomcn_windows_code_page(size_t* __p_maybe_dst_len, char** __p_maybe_dst,
	...
	CHAR __default_char           = '?';
	auto __used_defaults = ::ztd::__idk_detail::__windows::__multibyte_to_widechar_used_char(
	...
	const int __win32_dst_len
	     = static_cast<int>(!__is_unbounded ? (*__p_maybe_dst_len > static_cast<size_t>(INT_MAX)
	                                        ...
	                                        : INT_MAX);
	for (size_t __intermediate_input_read = 1; __intermediate_input_read <= __intermediate_size;
	     ++__intermediate_input_read) {
