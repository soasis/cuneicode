	...
	static cnc_mcerr simdutf_utf##FROM_N##_to_utf##TO_N##_convert(cnc_conversion*,                             \
		...
		if (!is_counting_only && is_unbounded_write) {                                                        \
			if (assume_valid) {                                                                              \
				...
				input_bytes_size = 0;                                                                       \
				*p_output_bytes += write_byte_size;                                                         \
				return cnc_mcerr_ok;                                                                        \
			}                                                                                                \
			else {                                                                                           \
		...
		cnc_mcerr err      = cnc_c##FROM_N##snrtoc##TO_N##sn(                                                 \
               is_unbounded_write ? &output_size : nullptr, &output, &input_size, &input, state);          \
		if (!is_unbounded_write) {                                                                            \
			*p_output_bytes_size = output_size * sizeof(to_char_t);                                          \
		}                                                                                                     \
		if (!is_counting_only) {                                                                              \
...
extern bool cnc_shared_add_simdutf_to_registry(
	...
	// unchecked functions
	{
		...
		cnc_open_err err = cnc_registry_add_multi_c8(registry, from_code.data(), to_code.data(),
