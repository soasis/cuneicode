...
#define UTF_CONVERSION_BENCHMARK(FROM_N, TO_N)                                                     \
	...
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_registry_core(                 \
		...
		{                                                                                        \
			...
			const cnc_open_error err                                                            \
			     = cnc_new_registry(&raw_registry, CNC_REGISTRY_OPTIONS_NONE);                  \
			...
			const cnc_open_error conv_err                                                       \
			     = cnc_conv_new(registry.get(), (const ztd_char8_t*)u8"UTF-" #FROM_N,           \
		...
		for (auto _ : state) {                                                                   \
			...
			[[maybe_unused]] size_t output_size                                                 \
			     = output_data.size() * sizeof(*output_data.data());                            \
			unsigned char* output = (unsigned char*)output_data.data();                         \
			cnc_mcerror err = cnc_conv(conversion.get(), Unbounded ? nullptr : &output_size,    \
		...
		if (!result || !is_equal) {                                                              \
			state.SkipWithError("bad benchmark result");                                        \
                                                                                                   ...
                                                                                                   \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_registry(                      \
	     benchmark::State& state) {                                                               \
		utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_registry_core<true, false>(state);    \
	}                                                                                             \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_registry_unbounded(            \
	     benchmark::State& state) {                                                               \
