...
static void utf8_to_utf16_well_formed_icu_u_strFromUTF8(benchmark::State& state) {
	std::vector<ztd_char8_t> input_data(c_span_char8_t_data(u8_data),
	     c_span_char8_t_data(u8_data) + c_span_char8_t_size(u8_data));
	icu::UnicodeString str(
	     (int32_t)c_span_char16_t_size(u16_data) + 1, 0, (int32_t)c_span_char16_t_size(u16_data));
	bool result = true;
	for (auto _ : state) {
		UErrorCode err     = U_ZERO_ERROR;
		int32_t output_len = (int32_t)(str.length() + 1);
		[[maybe_unused]] UChar* output_ptr
		     = u_strFromUTF8((UChar*)str.getBuffer(), (int32_t)output_len, &output_len,
		          (const char*)input_data.data(), (int32_t)input_data.size(), &err);
		if (!U_SUCCESS(err)) {
			result = false;
		}
	}
	const bool is_equal = std::equal(str.getBuffer(), str.getBuffer() + str.length(),
	     c_span_char16_t_data(u16_data),
	     c_span_char16_t_data(u16_data) + c_span_char16_t_size(u16_data));
	if (!result || !is_equal) {
		state.SkipWithError("bad benchmark result");
...
static void utf32_to_utf16_well_formed_icu_u_strFromUTF32(benchmark::State& state) {
	std::vector<ztd_char32_t> input_data(c_span_char32_t_data(u32_data),
	...
	for (auto _ : state) {
		...
		int32_t output_len = (int32_t)(str.length() + 1);
		[[maybe_unused]] UChar* output_ptr
		     = u_strFromUTF32((UChar*)str.getBuffer(), (int32_t)output_len, &output_len,
...
static void utf16_to_utf32_well_formed_icu_ucnv_fromUChars(benchmark::State& state) {
	...
	{
		UErrorCode open_err  = U_ZERO_ERROR;
		UConverter* raw_conv = ucnv_open(
		     ztd::endian::native == ztd::endian::big ? "UTF-32BE" : "UTF-32LE", &open_err);
		if (!U_SUCCESS(open_err)) {
			state.SkipWithError("bad benchmark result");
			return;
		}
	...
	const bool is_equal
	     = std::equal(output_data.cbegin(), output_data.cend() - 1, c_span_char32_t_data(u32_data),
	          c_span_char32_t_data(u32_data) + c_span_char32_t_size(u32_data));
	if (!result || !is_equal) {
		state.SkipWithError("bad benchmark result");
	}
...
static void utf32_to_utf16_well_formed_icu_ucnv_toUChars(benchmark::State& state) {
	...
	for (auto _ : state) {
		UErrorCode err     = U_ZERO_ERROR;
		int32_t output_len = (int32_t)(str.length() + 1);
		int32_t len        = ucnv_toUChars(conv.get(), (UChar*)str.getBuffer(), output_len,
		            (const char*)input_data.data(), (int32_t)(input_data.size() * sizeof(ztd_char32_t)),
		            &err);
		if (!U_SUCCESS(err)) {
			result = false;
		}
...
static void utf8_to_utf16_well_formed_icu_ucnv_toUChars(benchmark::State& state) {
	...
	for (auto _ : state) {
		...
		int32_t output_len = (int32_t)(str.length() + 1);
		int32_t len        = ucnv_toUChars(conv.get(), (UChar*)str.getBuffer(), output_len,
		            (const char*)input_data.data(), (int32_t)(input_data.size() * sizeof(ztd_char8_t)),
		            &err);
		if (!U_SUCCESS(err)) {
			result = false;
		}
...
#define UTF_CONVERSION_BENCHMARK(                                                                  \
     FROM_N, TO_N, BIG_FROM_SUFFIX, LIL_FROM_SUFFIX, BIG_TO_SUFFIX, LIL_TO_SUFFIX)                 \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_icu_ucnv_convertEx(                      \
	     benchmark::State& state) {                                                               \
		std::vector<ztd_char##FROM_N##_t> input_data(                                            \
		     c_span_char##FROM_N##_t_data(u##FROM_N##_data),                                     \
		     c_span_char##FROM_N##_t_data(u##FROM_N##_data)                                      \
		          + c_span_char##FROM_N##_t_size(u##FROM_N##_data));                             \
		...
		{                                                                                        \
			UErrorCode open_err  = U_ZERO_ERROR;                                                \
			UConverter* raw_conv = ucnv_open(ztd::endian::native == ztd::endian::big            \
			          ? "UTF-" #FROM_N #BIG_FROM_SUFFIX                                         \
		...
		for (auto _ : state) {                                                                   \
			...
			ucnv_convertEx(to_conv.get(), from_conv.get(), &output, output + output_size,       \
			     &input, input + input_size, nullptr, nullptr, nullptr, nullptr, true, true,    \
			     &err);                                                                         \
			if (!U_SUCCESS(err)) {                                                              \
				result = false;                                                                \
			}                                                                                   \
		...
		const bool is_equal = std::equal(output_data.cbegin(), output_data.cend(),               \
		     c_span_char##TO_N##_t_data(u##TO_N##_data),                                         \
		     c_span_char##TO_N##_t_data(u##TO_N##_data)                                          \
