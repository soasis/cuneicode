...
#define UTF_CONVERSION_BENCHMARKS(                                                                 \
	...
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_iconv(benchmark::State& state) {         \
		...
		{                                                                                        \
			...
			if (ztd::endian::native == ztd::endian::little) {                                   \
				raw_convd = iconv_functions.open(                                              \
				     "UTF-" #TO_N #LIL_TO_SUFFIX, "UTF-" #FROM_N #LIL_FROM_SUFFIX);            \
			}                                                                                   \
			else {                                                                              \
				raw_convd = iconv_functions.open(                                              \
				     "UTF-" #TO_N #BIG_TO_SUFFIX, "UTF-" #FROM_N #BIG_FROM_SUFFIX);            \
			}                                                                                   \
		...
		for (auto _ : state) {                                                                   \
			...
			size_t conv_result = iconv_functions.convert(                                       \
			     convd.get(), &input, &input_size, &output, &output_size);                      \
