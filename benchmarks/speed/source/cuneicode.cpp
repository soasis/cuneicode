...
#define UTF_CONVERSION_BENCHMARK(FROM_N, TO_N)                                                     \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode(benchmark::State& state) {     \
		...
		for (auto _ : state) {                                                                   \
			...
			cnc_mcerror err                                                                     \
			     = cnc_c##FROM_N##sntoc##TO_N##sn(&output_size, &output, &input_size, &input);  \
		...
		if (!result || !is_equal) {                                                              \
			state.SkipWithError("bad benchmark result");                                        \
                                                                                                   ...
                                                                                                   \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_single(                        \
		...
		for (auto _ : state) {                                                                   \
			...
			for (const ztd_char##FROM_N##_t* const input_last = input + input_size;             \
			     input != input_last;) {                                                        \
				cnc_mcerror err = cnc_c##FROM_N##nrtoc##TO_N##n(                               \
	...
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_unbounded(                     \
	     benchmark::State& state) {                                                               \
		const std::vector<ztd_char##FROM_N##_t> input_data(                                      \
		     c_span_char##FROM_N##_t_data(u##FROM_N##_data),                                     \
		     c_span_char##FROM_N##_t_data(u##FROM_N##_data)                                      \
		...
		for (auto _ : state) {                                                                   \
			...
			ztd_char##TO_N##_t* output        = output_data.data();                             \
			cnc_mcerror err                                                                     \
			     = cnc_c##FROM_N##sntoc##TO_N##sn(nullptr, &output, &input_size, &input);       \
			if (err != CNC_MCERROR_OKAY) {                                                      \
				result = false;                                                                \
			}                                                                                   \
		}                                                                                        \
		...
		if (!result || !is_equal) {                                                              \
			state.SkipWithError("bad benchmark result");                                        \
                                                                                                   ...
                                                                                                   \
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_cuneicode_single_unbounded(              \
	     benchmark::State& state) {                                                               \
		const std::vector<ztd_char##FROM_N##_t> input_data(                                      \
		     c_span_char##FROM_N##_t_data(u##FROM_N##_data),                                     \
		     c_span_char##FROM_N##_t_data(u##FROM_N##_data)                                      \
		...
		for (auto _ : state) {                                                                   \
			...
			for (const ztd_char##FROM_N##_t* const input_last = input + input_size;             \
			     input != input_last;) {                                                        \
				cnc_mcerror err = cnc_c##FROM_N##nrtoc##TO_N##n(                               \
				     nullptr, &output, &input_size, &input, &state);                           \
				if (err != CNC_MCERROR_OKAY) {                                                 \
					result = false;                                                           \
					break;                                                                    \
				}                                                                              \
