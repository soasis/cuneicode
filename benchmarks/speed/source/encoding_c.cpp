...
bool encoding_c_convert_through_utf8(encoding_rs::Decoder* decoder, encoding_rs::Encoder* encoder,
	...
	const auto native_utf16_encoding
	     = ztd::endian::native == ztd::endian::big ? UTF_16BE_ENCODING : UTF_16LE_ENCODING;
	if (decoder->encoding() == UTF_8_ENCODING) {
		...
		std::tie(encoder_result, encoder_read, encoder_written)
		     = encoder->encode_from_utf8_without_replacement(
		...
		size_t encoder_written  = 0;
		std::tie(encoder_result, encoder_read, encoder_written)
		     = encoder->encode_from_utf16_without_replacement(
		          std::u16string_view(
		               (const char16_t*)input.data(), input.size() / sizeof(char16_t)),
		          output, true);
		if (encoder_result != INPUT_EMPTY) {
			return false;
		}
	...
	for (;;) {
		...
		std::tie(decoder_result, decoder_read, decoder_written)
		     = decoder->decode_to_utf8_without_replacement(
		          input.subspan(decoder_offset), pivot_buffer, false);
...
#define UTF_CONVERSION_BENCHMARK(                                                                  \
	...
	static void utf##FROM_N##_to_utf##TO_N##_well_formed_encoding_c(benchmark::State& state) {    \
		const std::vector<ztd_char##FROM_N##_t> input_data(                                      \
		...
		for (auto _ : state) {                                                                   \
			bool successful = encoding_c_convert_through_utf8(decoder.get(), encoder.get(),     \
			     ztd::span<const unsigned char>((const unsigned char*)input_data.data(),        \
			          input_data.size() * sizeof(ztd_char##FROM_N##_t)),                        \
			     ztd::span<unsigned char>((unsigned char*)output_data.data(),                   \
			          output_data.size() * sizeof(ztd_char##TO_N##_t)));                        \
			if (!successful) {                                                                  \
				result = false;                                                                \
			}                                                                                   \
		}                                                                                        \
